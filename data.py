...
@st.cache_data(ttl=86400, show_spinner=False)
def fetch_inadimplentes_ref30d(atendente: str = None, situacao: str = "todos", _dia: str | None = None) -> int:
    """Conta clientes DISTINTOS que tinham cobranca em aberto em D-30 (30 dias atras).

    Mesma base do card operacional "X INADIMPLENTES" na tela atividades:
    todas as cobrancas (SEM filtro de tipo 1.2.1/1.2.2, SEM exclusao de
    onboarding), respeita filtros de atendente + situacao.

    "Em aberto em D-30" = venceu antes de D-30 E (ainda esta em aberto agora
    OU foi liquidada depois de D-30). Snapshot historico honesto.

    Usado para o delta MoM absoluto do card operacional — bate exato com o
    numero atual mostrado (total_inad_n = clientes com cobranca em aberto,
    respeitando os mesmos filtros).
    """
    client = get_bq_client()
    if not client:
        return 0
    ...
    if atendente == "__SEM_ESPECIALISTA__":
        atend_cond = """
        AND CAST(c.id_sacado_sac AS STRING) NOT IN (
          SELECT DISTINCT CAST(id_sacado_sac AS STRING)
          FROM `business-intelligence-467516.Splgc.splgc-grupo`
          WHERE grupo IS NOT NULL
        )"""
        ...
        ate_safe = atendente.replace("'", "''")
        atend_cond = f"""
        AND CAST(c.id_sacado_sac AS STRING) IN (
          SELECT DISTINCT CAST(id_sacado_sac AS STRING)
          FROM `business-intelligence-467516.Splgc.splgc-grupo`
          WHERE grupo = '{ate_safe}'
        )"""
    ...
    sit_cond = ""
    if situacao == "ativos":
        sit_cond = "AND c.dt_desativacao_sac IS NULL"
    elif situacao == "inativos":
        sit_cond = "AND c.dt_desativacao_sac IS NOT NULL"

    query = f"""
    SELECT COUNT(DISTINCT c.id_sacado_sac) AS n
    FROM `business-intelligence-467516.Splgc.splgc-cobrancas_competencia-all` c
    LEFT JOIN `business-intelligence-467516.Splgc.splgc-cobrancas_liquidacao-all` l
      ON c.id_recebimento_recb = l.id_recebimento_recb
    WHERE DATE(c.dt_vencimento_recb) < DATE('{ref_str}')
      AND (c.fl_status_recb = '0' OR DATE(l.dt_liquidacao_recb) >= DATE('{ref_str}'))
      {atend_cond}
      {sit_cond}
    """
    try:
