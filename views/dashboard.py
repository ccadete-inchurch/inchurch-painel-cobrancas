...
def _render_dashboard(store, clientes, role):
    ...
    # ── Regularizados hoje (todos, inclusive fora-lote) ─────────────────────
    # Mostra pagamentos TOTAIS do dia em uma tabela compacta. Parciais ficam
    # invisíveis (saldo do cliente cai silenciosamente). Atendente vê quem
    # da carteira dela pagou, mesmo que esteja fora do lote do dia.
    _hoje_dash = date.today().strftime("%d/%m/%Y")
    regs_hoje = sorted(
        [r for r in store.get("regularizados", []) if str(r.get("data") or "") == _hoje_dash],
        key=lambda r: float(r.get("valor") or 0),
        reverse=True,
    )
    if regs_hoje:
        ...
        # Cabeçalho da tabela
        col_w_r = [3, 1.8, 1.3, 1.5]
        hdrs_r = ["Cliente", "CNPJ", "Valor pago", "Atendente"]
        hdr_html_r = "".join(
            f'<div style="flex:{w};padding:12px 14px;font-size:11px;text-transform:uppercase;'
            f'letter-spacing:1.2px;color:#8b94a5;font-weight:700">{h}</div>'
            for w, h in zip(col_w_r, hdrs_r)
        )
        st.markdown(
            f'<div style="display:flex;gap:1rem;background:#1e2333;border:1px solid #2a2f42;'
            f'border-radius:10px 10px 0 0;overflow:hidden">{hdr_html_r}</div>',
            unsafe_allow_html=True,
        )

        # Linhas (limita 50 — em volume maior, atendente vai pra aba Regularizados)
        _MAX_LINHAS = 50
        _linhas = regs_hoje[:_MAX_LINHAS]
        for i, r in enumerate(_linhas):
            inativo_b = (
                '<span style="background:#6b7280;color:#fff;font-size:9px;font-weight:700;'
                'padding:1px 6px;border-radius:3px;margin-left:6px;vertical-align:middle">INATIVO</span>'
                if r.get("inativo") else ""
            )
            rcols = st.columns(col_w_r)
            with rcols[0]:
                st.markdown(
                    f'<div style="padding:10px 14px;font-size:14px;color:#e8eaf0;font-weight:600">'
                    f'{r.get("nome","—")}{inativo_b}</div>',
                    unsafe_allow_html=True,
                )
            with rcols[1]:
                st.markdown(
                    f'<div style="padding:10px 14px;font-size:13px;color:#9ca3af">{r.get("cnpj","—")}</div>',
                    unsafe_allow_html=True,
                )
            with rcols[2]:
                st.markdown(
                    f'<div style="padding:10px 14px;font-size:14px;font-weight:700;color:#7cc243">'
                    f'{fmt_moeda_plain(float(r.get("valor", 0) or 0))}</div>',
                    unsafe_allow_html=True,
                )
            with rcols[3]:
                _at = r.get("atendente") or "—"
                # 'Sistema (BigQuery)' vem do histórico — encurta pra "—"
                if "BigQuery" in str(_at):
                    _at = "—"
                st.markdown(
                    f'<div style="padding:10px 14px;font-size:13px;color:#9ca3af">{_at}</div>',
                    unsafe_allow_html=True,
                )
            if i < len(_linhas) - 1:
                st.markdown('<div style="height:0.5px;background:#2a2f42;margin:0"></div>', unsafe_allow_html=True)

        if len(regs_hoje) > _MAX_LINHAS:
            st.markdown(
                f'<div style="font-size:12px;color:#6b7280;text-align:center;padding:10px;'
                f'background:#1e2333;border:1px solid #2a2f42;border-top:none;'
                f'border-radius:0 0 10px 10px">+ {len(regs_hoje) - _MAX_LINHAS} pagamentos · '
                f'ver lista completa na aba Regularizados</div>',
                unsafe_allow_html=True,
            )
        else:
            # Fecha a borda inferior da tabela quando cabe tudo
            st.markdown(
                '<div style="height:1px;background:#2a2f42;border-radius:0 0 10px 10px"></div>',
                unsafe_allow_html=True,
            )
