...
def _render_card(score, acoes, c, role, idx):
    ...
    st.markdown(
        ...
        f'<div style="font-size:11px;color:#9ca3af;font-weight:400;margin-top:4px;line-height:1.7">'
        f'CNPJ {c.get("cnpj","—")}<br>ID {c.get("id","—")}'
        f'</div>'
        f'</div>'
        f'<div style="text-align:right;flex-shrink:0">'
        f'<div style="font-size:20px;font-weight:800;color:{cor};line-height:1">{score}</div>'
        f'<div style="font-size:12px;color:#6b7280">pts</div>'
        f'</div></div>'
